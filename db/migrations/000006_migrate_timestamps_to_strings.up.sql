...
ALTER TABLE anime ADD COLUMN formatted_timestamp VARCHAR(255);
UPDATE anime SET formatted_timestamp = to_char(end_date, '%Y-%m-%d %H:%i:%s') WHERE end_date IS NOT NULL;
...
UPDATE episodes SET formatted_timestamp = to_char(aired, '%Y-%m-%d %H:%i:%s') WHERE aired IS NOT NULL;
